...
exports.createAssignment = async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      const courseName = course?.Course_Name || course?.Course_Initial || "Course";
      ...
      await createNotificationsForCourse(
        ...
        `A new assignment "${assignmentName}" has been posted in ${courseName}. Deadline: ${deadlineDate}`,
        assignment._id.toString()
      );
    } catch (notifErr) {
      console.error("Error creating notifications for assignment:", notifErr);
...
exports.submitAssignmentFeedback = async (req, res) => {
  ...
  try {
    ...
    // Create notification for the student who submitted the assignment
    try {
      const assignmentAnswer = await AssignmentAnswer.findById(assignmentAnswerId);
      if (assignmentAnswer) {
        const assignmentQuestion = await AssignmentQuestion.findById(assignmentAnswer.assignmentQuestionId);
        if (assignmentQuestion) {
          const course = await Course.findById(assignmentQuestion.courseId);
          const courseName = course?.Course_Name || course?.Course_Initial || "Course";
          
          await createNotificationForStudent(
            assignmentAnswer.studentId,
            "feedback",
            "Assignment Feedback Received",
            `You have received feedback on your assignment submission in ${courseName}`,
            assignmentQuestion.courseId,
            courseName,
            newFeedback._id.toString()
          );
        }
      }
    } catch (notifErr) {
      console.error("Error creating notification for feedback:", notifErr);
